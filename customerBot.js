...
const { TelegramBot } = require('node-telegram-bot-api');const { createClient } = require('@supabase/supabase-js');
...
const CONFIG = Object.freeze({
  ...
  adminIds:         (process.env.ADMIN_IDS || '').split(',').map((s) => s.trim()).filter(Boolean), // آيديات تيليجرام للأدمن المسموح له بعمل broadcast
  broadcastDelayMs: Number(process.env.BROADCAST_DELAY_MS || 40), // تأخير بين كل رسالة broadcast (لتجنب حظر تيليجرام)
});
...
async function getAllCustomerIds() {
  const { data, error } = await supabase.from('customers').select('telegram_id');
  if (error) throw error;
  return (data ?? []).map((c) => c.telegram_id).filter(Boolean);
}
...
// ══════════════════════════════════════════════
// 12b. PRODUCTS — MEDIA GROUP + PAGINATION
// ══════════════════════════════════════════════

async function showProductsPaged(chatId, merchantId, category = null, page = 0) {
  const pageSize = 4;
  const products = await getProducts(merchantId, category === 'all' ? null : category);
  if (!products.length) {
    return send(chatId, 'لا توجد منتجات متاحة.', {
  ...
  await send(chatId, `🛍 <b>المنتجات${label}</b>  (${products.length}) — الصفحة ${page + 1}/${totalPages}`);

  // تقسيم: منتجات بصور / بدون صور
  const withImages    = slice.filter((p) => p.image_file_id || p.image_channel_id);
  const withoutImages = slice.filter((p) => !p.image_file_id && !p.image_channel_id);

  // ── Media Group للمنتجات التي لها صور ──
  if (withImages.length >= 2) {
    const media = withImages.map((p, idx) => {
      const entry = {
        type:  'photo',
      ...
      if (idx === 0) {
        entry.caption =
          withImages
            .map((x, j) => `${j + 1}. <b>${esc(x.name)}</b> — ${formatAmount(x.price)}`)
            .join('\n');
    ...
    try {
      ...
      for (const p of withImages) await sendProductCard(chatId, p);
    }

    // أزرار إضافة لكل منتج في الـ album
    const addRows = withImages.map((p) => [
      { text: `➕ ${esc(p.name)} — ${formatAmount(p.price)}`, callback_data: `add:${p.id}` },
    ]);
    await send(chatId, '👆 اختر منتجاً للإضافة:', { reply_markup: kb.inline(addRows) });

  } else if (withImages.length === 1) {
    await sendProductCard(chatId, withImages[0]);
  }

  // ── منتجات بدون صور كبطاقات نصية ──
  for (const p of withoutImages) await sendProductCard(chatId, p);

  // ── شريط التنقل ──
  const nav = [];
  if (page > 0)
    nav.push({ text: '◀ السابق', callback_data: `products_page:${merchantId}:${catKey}:${page - 1}` });

  nav.push({ text: `${page + 1} / ${totalPages}`, callback_data: 'noop' });

  if (page < totalPages - 1)
    nav.push({ text: 'التالي ▶', callback_data: `products_page:${merchantId}:${catKey}:${page + 1}` });

  await send(chatId, '─────────────────', {
    reply_markup: kb.inline([
      nav,
      [{ text: '🛒 سلتي', callback_data: 'show_cart' },
       { text: '« رجوع للتصنيفات', callback_data: `store:${merchantId}` }],
    ]),
  });
}
...
// ══════════════════════════════════════════════
// 18b. BROADCAST (تنبيه كل الزبائن بتحديث جديد)
// ══════════════════════════════════════════════
...
async function broadcastMessage(adminChatId, text) {
  let ids;
  try {
    ids = await getAllCustomerIds();
  } catch (err) {
    logError('broadcast:getIds', err);
    return send(adminChatId, '❌ فشل جلب قائمة الزبائن.');
  }

  if (!ids.length) return send(adminChatId, '⚠️ لا يوجد زبائن مسجلين.');

  await send(adminChatId, `📣 بدء الإرسال إلى <b>${ids.length}</b> زبون...\n⏳ هذا قد يستغرق بعض الوقت.`);
  ...
  for (const id of ids) {
    try {
      // إرسال الرسالة + تحديث الكيبورد تلقائياً عند الزبون بدون أي تدخل منه
      await bot.sendMessage(id, text, { parse_mode: 'HTML', reply_markup: kb.main() });
      sent++;
    } catch (err) {
      const code = err?.response?.body?.error_code;
      if (code === 403) blocked++; // الزبون حاظر/مسكر البوت
      failed++;
      logError('broadcast:send', err);
    }
    await sleep(CONFIG.broadcastDelayMs); // تأخير لتجنب حدود تيليجرام (flood limits)
  }
...
bot.onText(/\/start/, async (msg) => {
  ...
  // تنبيه الزبون بوجود متجر إلكتروني (Mini App) عبر زر القائمة بجانب حقل الكتابة
  if (CONFIG.webAppUrl) {
    await send(
      chatId,
      `💡 على يسارك زر ازرق اضغط علية ليحولك الى المحلات `
    );
  }
});
...
bot.onText(/^\/broadcast(?:\s+([\s\S]+))?$/, async (msg, match) => {
  ...
  if (!isAdmin(msg.from?.id)) return; // تجاهل تام لغير الأدمن (حتى لا يعرف أحد بوجود الأمر)

  const text = match?.[1]?.trim();
  if (!text) {
    return send(
      chatId,
      '⚠️ اكتب الرسالة بعد الأمر، مثال:\n\n<code>/broadcast 🎉 أضفنا متجراً جديداً! جرب يلا</code>'
    );
  }

  broadcastMessage(chatId, text).catch((e) => logError('/broadcast', e));
});
...
bot.on('callback_query', async (q) => {
 ...
 if (action.startsWith('products:')) {
      ...
      return showProductsPaged(chatId, merchantId, cat, 0);
    }
    if (action.startsWith('products_page:')) {
      const [, merchantId, cat, pageStr] = action.split(':');
      return showProductsPaged(chatId, merchantId, cat, Number(pageStr));
    }
